...
\teorema{}{Sia $f: D \subset \mathbb{R}^{2} \rightarrow  \mathbb{R}$ e sia $P_0=(x_0,y_0) \in D$ allora:
    ...
    uniformenmente rispetto a $\theta$
...
\begin{proof}
       per far vedere che vale il limite e' sufficiente mostrare che esiste una funzione $g$ che dipende solo da $\rho$ (non negativa) $g(\rho)\ge 0$ tale che:
       ...
       e poi faccio vedere che quindi (per il teorema dei due caramba):
...
\textbf{Esempio gia' visto} 

Avevamo gia mostrato che il limite non esiste:
...
trasformiamo in coordinate polri:
...
Calcolare se esistono i sequenti limiti e far vedere che non esistono:
...
dove $M(\theta)= \frac{cos^{4}\theta}{sin\theta}$ si nota che per ogni $\theta$ finito il limite e' zero. Candidato limite e' 0
...
se la nostra funzione e' limitata per ogni $\theta$ allora:

\[
    |M(\theta)| \le \bar{e} 
\]

il sup cioe' e' finito sono a posto ma $M(\theta)$ non e' limitata (per esempio $\theta= \pi$ e' un asintoto verticale):

\[
    \lim_{ \theta \to \pi } \frac{cos^{4}\theta}{sin\theta}= +\infty
\]

allora per studiare il limite vediamo che sucede muovendoci verso l'origine lungo curve che sono tangenti all'asse x.
...
poiche $|cos^{2}\theta sin\theta|\le 1$ se dunque $3-2 \alpha >0$ si ha:
...
Studiamo la continuita della funzione nel sottoinsieme di definizione:
...
Nel piano $(y,z)$ la sezione e' data dal grafico di:
...
Consideriamo i punti $(x,y)$ che stanno nell'insieme di livello $k=1$ della funzoine $f(x,y)=x^{2}+y^{2}$:
...
Determinare l'insieme di definizione delle sequenti funzioni:
...
se $k=0$, e' $E_0$ privata dell'origine:
...
quindi sono parabole con concavita verso l'alto.
...
se $k<0$ avranno concavita verso il basso
...
e quindi il limite di $g(x,y)$ (dato che la funzione e' continua):
...
il numeratore di questa, se trasformiamo in coordinate polari, e' come $\rho^{3}$, controllo quindi il denominatore:
...
ergo il limite non esiste perche $f= f_1+f_2$ e $\lim_{ (x,y) \to (0,0) } f_2=0$ e $\lim_{ (x,y) \to (0,0) } f_1$ non esiste.
...
\begin{itemize}
    ...
    \item $x=y^{2}$ il denomitaore ($x^{2}+2y^{4}$) ottengo $y^{4}+2y^{4} = 3y^{4}$ quindi qua non trascure gli addendi.
\end{itemize}
...
\subsection{Calcolo differenziale di funzioni di piu variabili}
...
se esiste ed e' finito allora f e' derivabile in $x_0$ ed $f'(x_0) = limite$.
...
Vogliamo calolare la derivata parziale di $f$ fatta rispetto a $x$ calcolata in $(x_0,y_0)$:

\[
    \lim_{ h \to 0 } \frac{f(x_0+h,y_0) -f(x_0,y_0)}{h}
\]

purche' esista, finito.
...
se tale limite esiste ed e' finito.
...
questa e' una funzione che dipende solo da $x$ (quindi diventa di una sola variabile), se questa e' derivabile in $x_0$ cioe':
...
infatti torna uguale all'atro metodo.
...
Per le funzioni in due variabili la derivabilita' non implica la continuita':
...
Dunque ottengo due funzioni identicamente nulle che sono derivabil con derivata nulla:

\[
    f_x(0,0) = f_y(0,0) = 0
\]

Osserviamo pero che in $(0,0)$ la funzione non e' continua perche':

\[
    \lim_{ (x,y) \to (0,0) } \frac{xy^{2}}{x^{2}+y^{4}} \text{ non esiste}
\]

quello che ci servira' sara' il concetto di \textbf{differenziabilita'}.

\subsection{Gradiente}

Se $f: A \subset \mathbb{R}^{2}\rightarrow \mathbb{R}$ e' derivabile in $P_0=(x_0,y_0)$:

\[
    \nabla f(x_0,y_0)=(\frac{\partial f}{\partial x}(x_0,y_0), \frac{\partial f}{\partial y}(x_0,y_0)) \in \mathbb{R}^{2}
\]

gradiente di $f$ in $P_0$ (e' un vettore).
...
L'equazione del piano tangente alla funzione nel punto $P_0$ e':
...
dove $S$ e' il grafico della funzione
\[
...
\subsection{Differenziabilita'}

Per dire che $z = f(x_0,y_0) + f_x(x_0,y_0) (x-x_0) + f_y(x_0,y_0) (y-y_0) $ e' l'equazione del piano tangente, devo vedere anche la differenziabilita' (il piano contiene le rette $T_1$ e $T_2$ che hanno in comune $P_0$).
...
\defn{Curva}{
Una curva e' un'applicazione continua:
...
l'immagine della curva e' chiamata sostegno della curva:
...
Sia $f:[a,b]\rightarrow \mathbb{R}$ continua il suo grafico e' il sostegno della curva piana data da:
...
\defn{Curva regolare}{
Una curva si dice regolare se l'applicazione $\varphi$ e' di classe $C^{1}$ (le derivate prime sono continue) e $\varphi'(t) \neq 0$
...
allora la retta tangente alla curva nel punto $\varphi(t_0)$ e' proprio la retta tangente al grafico di f nel punto $(x_0,f(x_0))=(t_0,f(t))$

Se $\varphi(t)$ e' regolare allora il vettore $\varphi'(t_0)$ si chiama vettore tangente alla curva nel punto $\varphi(t_0) \in \mathbb{R}^{3}$
...
Il sostegno delle due curve e' lo stesso vanno entrambe a $\mathbb{R}^{2}$

Le curve pero' sono diverse perche il dominio e' diverso.
...
nel modo sequente, definisco $\forall i = 1,...,n$:

\[
    \bar{e_i}  = (0,...0,1,0,....,0) 
\]

quindi la derivata e' data dal sequente limite purche esista e sia finito:
...
Se g e' derivabile allora si ha:
...
dire che quindi $f$ e' derivabile in $\bar{x} \in A$ e' come dire che esiste il vettore gradicente:
