...
\proposizione{}{Se $f: A \subseteq \mathbb{R}^{n}\rightarrow \mathbb{R}$, $A$ aperto, $x_0 \in A$.
    ...
    \begin{itemize}
        \item Se $f$ è differenziabile in $x_0$ $\rightarrow$ $f$ è continua in $x_0$
        \item Se $f$ è differenziabile in $x_0$ $\rightarrow$ $f$ ha tutte le derivate direzionali in $x_0$ (secondo tutte le direzioni $\bar{v} $) e si ha $\frac{\partial f}{\partial v}(x_0) = D_{ \bar{v} } f(x_0) = L(\bar{v} ), \forall v \in \mathbb{R}^{n} $

            In particolare, $\bar{v} \rightarrow \frac{\partial f}{\partial v}(x_0)$ è un'applicazione lineare e il differenziale è unico (se esiste).
    \end{itemize}
}
...
consideriamo una curva continua $\gamma : [-1,1] \subset \mathbb{R} \rightarrow A\subseteq \mathbb{R}^{n}\ curva$ e supponiamo $\gamma(t)$ vettore (di $n$ componenti) sia derivabile, cioe':
...
questo si chiama \textbf{teorema delle derivate delle funzioni compose} o regola della catena.

}

dunque la crescita di $f$ in $x_0$ lungo una qualsiasi curva $\gamma$ regolare ($\gamma \in \mathbb{C}^{(1)}$) uscente da $x_0$ con velocita' $\gamma'(0) = v$ dipende dalla curva solamente attraverso il vettore (tangente) velovita' $\bar{v} $.
...
\begin{proof}
       Dimostrato dalle considerazioni fatte fino ad ora.    
\end{proof}

\textbf{Proprieta'} 

Se non è nulla, la lunghezza del gradiente di una funzione differenziabile in un punto, indica la direzione e il verso di \textbf{massima pendenza}  del grafico di $f$ nel putno. E dunque se considero la derivata direzionale di $f$ (nel punto) nella direzione del vettore gradiente trovo il valore della massima pendenza, che è appunto la lunghezza del vettore gradiente $|\nabla f(x_0)|$:
