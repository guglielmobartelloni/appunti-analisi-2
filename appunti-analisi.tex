...
L'altra volta abbiamo dimostrato che se abbiamo due soluzioni $y_1$ e $y_2$ esse sono linearmente indipendenti cioe' il determinante della matrice di $y_1(x)y_2'(x)-y_2(x)y_2'(x)$ e' diverso da 0 (determinante doskriamo?)
...
La sua equazione associata $a(\lambda)$:
...
\begin{proof}
    ...
    Se considero $y(x) = e ^{\lambda x}$ si ha:
...
altrimenti questo caso corrisponde a soluzioni complesse coniugate  
...
\begin{proof}
       ...
       scrivo la wronskiana di $y_1,y_2$:
        ...
        io devo determinare $u(x)$ per poi dimostrare che $y(x) = c_1 ^{\lambda_1 x}+c_2 e^{\lambda_2 x}$
