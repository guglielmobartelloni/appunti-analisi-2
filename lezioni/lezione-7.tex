...
Altra cosa interessante:
 ...
 \proposizione{Disuguaglianza di Cauchy-Schwarz}{
     ...
     \[
         |x\bullet y| = ||x||\cdot ||y||  \Leftrightarrow y = 0 \lor x=\lambda y \text{ con } \lambda \in \mathbb{R}\ge 0
     \]

     cioè solo se uno dei vettori è un multiplo scalare del l'altro.
 ...
 \begin{proof}
     ...
     \[
         |x + ty| ^{2} \overset{\text{Carnot}}{=} |x|^{2} + |ty|^{2} + 2\langle x,ty \rangle  = |x|^{2} + |y|^{2}t^{2} + 2 \langle x,y \rangle t \ge 0
     \]

     è un polinomio di II grado in t dove $|y|^{2}> 0 $ essendo $y \neq 0$

     Il nostro $\frac{\Delta}{4}$ deve essere non positivo (se no in qualche punto il polinomio dovrebbe essere negativo ma abbiamo supposto che sia sempre positivo):
