...
\begin{enumerate}
    \item Si cercano le soluzioni costanti $g(y)=0$ cioè gli zeri
        Si determinao gli eventuali $\bar y$ reali t.c. $g(\bar y)$

        $y(x)= \bar y$ sono soluzioni signolari del problema 
...
Avendo già risolto la EDO imponiamo la condiizione $y(0) = 3$:
...
\textbf{ Soluzione in piccolo (locale) } (è definita in un intervallo appartenente??? a 0)

\textbf{ Soluzione in grande (globale) } (è definita in tutto l'intervallo)
...
le soluzioni sono linearmente indipendenti
...
io so che le soluzioni soddisfano la equazione (ovviamente):
