...
\teorema{}{Consideriamo l'applicazione verticale $F(\rho,\theta) = \left(F_1(\rho,\theta), F_2(\rho,\theta)\right)$ in cui $F: A \rightarrow \mathbb{R}^{2}$ in cui $A = (0,+\infty) \times (0,2\pi)$.
    ...
    Sia $S \subset (0,+\infty) \times (0,2\pi)$ un aperto misurabile nel piano $(\rho,\theta)$ con $\bar{S} \subset (0,\infty) \cdot (0,2\pi)$ e sia $T=F(\bar{S})$ (fig. \ref{fig:polari})


Allora per ogni funzione $f$ integrabile su $T$, continua e limitata, vale la sequente formula:

\[
  \iint_{T=F(S)} {f(x,y)} \: d x d y  = \iint_{S=F^{-1}(T)} {f( \rho \cos \theta, \rho \sin \theta) \underbrace{\rho}_\text{det $JF(\rho,\theta)$}} \: d \rho d \theta  
\]

Più in generale in realtà:

\[
  \iint_{T=F(S)} {f(x,y)} \: d x d y  = \iint_{S=F^{-1}(T)} {f( F_1(\rho, \theta) , F_2(\rho, \theta)) \text{det $JF(\rho,\theta)$}} \: d \rho d \theta  
\]
...
\[
    \iint_{u^{2}+v^{2} \le 5} {v^{2}} \: dudv 
\]

\[
    D= \{(u,v) \in \mathbb{R}^{2}, u^{2}+v^{2} \le 5\}
\]
