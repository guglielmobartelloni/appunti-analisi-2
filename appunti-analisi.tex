...
\defn{Successione}{ Una successione è un elenco ordinato di numeri $\{x_n\} \subset \mathbb{R}^{n}$ (gli elementi della successione sono elementi di $\mathbb{R}^{n}$ ovvero n-plue di reali)
...
\proposizione{}{ Sia $\{x_n\} \subset \mathbb{R}^{n}$ una successione in $\mathbb{R}^{n}$. Si ha:
    ...
    \begin{enumerate}
        \item (\textbf{Unicita}) $\{x_n\}$ ha massimo un unico limite ( se $\{x_n\}$ ammette limite questo è unico)
...
f è derivabile in $\bar{x} =(x_1,...,x_n) \in A$ è lo stesso che dire che esiste:

\[
    \nabla f(\bar{x} ) = ( \frac{\partial f}{\partial x_1}(\bar{x} ),..., \frac{\partial f}{\partial x_n}(\bar{x} ) )
\]


\defn{}{ Si dice che $f$ è differenziabile in $\bar{x} \in A $ se $f$ è derivabile in $\bar{x} $ ed inoltre vale la sequente relazione:

    \[
        \lim_{ h \to 0 } \frac{f( \bar{x} + \bar{h} ) - f( \bar{x} ) - <\nabla f(\bar{x} ), \bar{h} >}{|\bar{h} |} =0
    \]


    dove $\bar{h} \in \mathbb{R}^{n}$ e in particolare $|\bar{h} | = \sqrt{\sum^{n}_{i=1} h_i^{2}}$, inlotre :

    \[
    <\nabla f(\bar{x} ), \bar{h} > = \frac{\partial f}{\partial x_1}(\bar{x} )h_1+ \frac{\partial f}{\partial x_2}(\bar{x} ) h_2+ ... + \frac{\partial f}{\partial x_n}(\bar{x} ) h_n
    \]
...
\[
    f(\bar{x} + \bar{h} ) = f(\bar{x} ) + <\nabla f(\bar{x} ), \bar{h} > + o(|h|)
\]
