...
\defn{Successione}{ Si dice che una successione ${x_n} \subset \mathbb{R}^{n}$ (gli elementi della successione sono elementi di $\mathbb{R}^{n}$ ovvero n-plue di reali)

    \[
        x_n= (x_n^{1}, x_n^{2}, ..., x_n ^{n})
    \]

    di dice che converge a $x \in \mathbb{R}^{n}$ se:
    ...
    \[
        \forall \epsilon >0 , \exists \bar{N} \in \mathbb{N}
    \]

    \[
        d(x_n, x ) < \epsilon, \forall n \ge \bar{N} 
    \]
...
\proposizione{}{ Sia ${x_n} \subset \mathbb{R}^{n}$ una successione in $\mathbb{R}^{n}$. Si ha:
    ...
    \begin{enumerate}
        \item (Unicita) ${x_n}$ ha massimo un unico limite ( se ${x_n}$ ammette limite questo e' unico)
        \item (Limitatezza) ogni successione convergente e' limitata: 
         ...
         \item (Sottosuccessione) Se ${x_n}$ convergente a $x \in  \mathbb{R}^{n}$ ( o in $X$) allora ogni sottosuccessione ${x_{n_k}}$ estratta da ${x_n}$ converge allo stesso limite
    \end{enumerate}
...
\defn{Punto esterno}{ $x_0 \in X$ si dice esterno ad $A$ ($A \subset X$) se non solo $x_0$ non appartiene ad $A$ ma vi e' almeno un suo intorno sferico completamente disgiunto ad $A$
    ...
    \[
        x_0 \in A \text{ e } \exists r>0\ B(x_0,r)\sqcup A = \emptyset
    \]
