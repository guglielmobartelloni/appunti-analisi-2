...
\begin{enumerate}
    ...
    \item 
        essendo $f \in \mathbb{C}^{1}(A)$ dove $A = \mathbb{R}^{2} \{(0,0)\} \rightarrow f \text{ è differenziabile}$, bisogna vedere che cosa succese in $(0,0)$
        ...
        provo a risolverlo ponenodo $y=x$:
...
\begin{proof}
     Per dimosttrare che $f$ è continua in $\bar{x_0} $ devo far vedere che:
...
\teorema{Teorema del differenziale}{ Sia $f: A \subseteq \mathbb{R}^{n} \rightarrow \mathbb{R}$, $A$ aperto, derivabile in $A$. 
    ...
    Se le derivate parziali $f_{x_1}, ..., f_{x_n}$ sono continue in $\bar{x} \in A$ allora $f$ è differenziabile in $\bar{x} $
...
\begin{proof}
       ...
       a questo punto faccio vedere la definizione di differenziabilita e sostituisco quello sopra:

       \[
           \frac{f(x+h,y+k) - f(x,y) - f_x(x,y)h - f_y(x,y) k}{\sqrt{h^{2}+k^{2}}} = \frac{f_x(x_1,y+k) h + f_y(x,y_1) k -f_x(x,y) - f_y(x,y) k}{\sqrt{h^{2}+k^{2}}} = 
       \]
       ...
       vediamo che succede quando $(h,k) \rightarrow (0,0)$:
        ...
        \[
              |\underbrace{f_x(x,y) - f_x(x,y)}_\text{0} | + |\underbrace{f_y(x,y) - f_y(x,y)}_\text{0} |
        \]
...
In generale $f \in \mathbb{C}^{k}(A), k \in \mathbb{N}$
